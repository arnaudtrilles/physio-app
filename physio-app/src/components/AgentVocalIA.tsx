...
export function AgentVocalIA({ agentId, toolCallbacks, onClose }: AgentVocalIAProps) {
  const { status, messages, error, isMuted, activePatient, start, stop, toggleMute } =
  ...
  return (
    <>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 16px' }}>
          ...
          <div style={{ position: 'relative', flexShrink: 0 }}>
            <div style={{
              ...
              boxShadow: isActive ? '0 0 16px 4px rgba(168,85,247,0.5)' : 'none',
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
              ...
              <span style={{ fontSize: '0.75rem', fontWeight: 700, color: 'white', letterSpacing: '0.02em' }}>
                Lucie
              </span>
              <span style={{ fontSize: '0.68rem', color: isActive ? '#c084fc' : 'rgba(255,255,255,0.3)', fontWeight: 500 }}>
                · {statusText[status]}
              </span>
            </div>

            {/* Dernière action système ou transcript */}
            {lastSystemMsgs.length > 0 && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                {lastSystemMsgs.map((msg, i) => (
                  <div key={i} style={{
                    fontSize: '0.7rem', color: 'rgba(216,180,254,0.7)',
                    whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
                    opacity: i === lastSystemMsgs.length - 1 ? 1 : 0.5,
                  }}>
                    {msg.text}
                  </div>
                ))}
            ...
            {lastSystemMsgs.length === 0 && lastAgentMsg && (
              <div style={{ fontSize: '0.7rem', color: 'rgba(216,180,254,0.6)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                {lastAgentMsg.text}
              </div>
            )}
            {error && (
              <div style={{ fontSize: '0.68rem', color: '#fca5a5', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                ⚠ {error}
              </div>
            )}
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            {/* Mute */}
            <button onClick={toggleMute} disabled={!isActive} aria-label={isMuted ? 'Réactiver' : 'Couper le micro'}
              style={{ width: 36, height: 36, borderRadius: '50%', border: `1px solid ${isMuted ? 'rgba(248,113,113,0.5)' : 'rgba(255,255,255,0.1)'}`, background: isMuted ? 'rgba(220,38,38,0.15)' : 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: isActive ? 'pointer' : 'default', color: isMuted ? '#fca5a5' : 'rgba(255,255,255,0.4)', opacity: isActive ? 1 : 0.35, transition: 'all 0.3s' }}>
            ...
            {/* Start/Stop */}
            <button onClick={isActive ? stop : start} disabled={status === 'connecting'}
              style={{ width: 44, height: 44, borderRadius: '50%', border: 'none', cursor: status === 'connecting' ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: status === 'connecting' ? 0.6 : 1, transition: 'all 0.3s',
                background: isActive ? 'linear-gradient(135deg, #dc2626, #7f1d1d)' : 'linear-gradient(135deg, #9333ea, #4f46e5)',
                boxShadow: isActive ? '0 0 18px rgba(220,38,38,0.5)' : '0 0 18px rgba(147,51,234,0.5)',
              }}>
              {status === 'connecting'
                ...
                : isActive
                  ? <svg width="16" height="16" viewBox="0 0 24 24" fill="white"><rect x="6" y="6" width="4" height="12" rx="1"/><rect x="14" y="6" width="4" height="12" rx="1"/></svg>
                  : <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2a3 3 0 0 0-3 3v7a3 3 0 0 0 6 0V5a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="22"/></svg>
              }
            </button>

            {/* Plein écran */}
            <button onClick={() => setExpanded(true)} aria-label="Agrandir"
              style={{ width: 36, height: 36, borderRadius: '50%', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'rgba(255,255,255,0.35)', transition: 'all 0.2s' }}>
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="18 9 12 3 6 9"/></svg>
            </button>

            {/* Fermer */}
            <button onClick={async () => { await stop(); onClose() }} aria-label="Fermer"
              style={{ width: 36, height: 36, borderRadius: '50%', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'rgba(255,255,255,0.35)', fontSize: '1.1rem', transition: 'all 0.2s' }}>
              ×
            </button>
          </div>
        </div>
      ...
      <div style={{ height: activePatient ? 130 : 105 }} />
