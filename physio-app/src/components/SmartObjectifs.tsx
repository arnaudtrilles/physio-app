...
export function SmartObjectifs({ objectifs, patientKey, onUpdate }: SmartObjectifsProps) {
  ...
  const statusColor = (s: SmartObjectif['status']) =>
    s === 'atteint' ? 'var(--color-success)' : s === 'non_atteint' ? 'var(--color-danger)' : 'var(--color-warning)'
  const statusBg = (s: SmartObjectif['status']) =>
    s === 'atteint' ? 'var(--color-success-bg2)' : s === 'non_atteint' ? 'var(--color-danger-bg)' : 'var(--color-warning-bg)'
    ...
    return (
      <div>
        <button
          ...
          style={{ width: '100%', padding: '0.7rem', borderRadius: 10, border: '2px dashed var(--color-warning-border)', background: 'transparent', color: 'var(--color-warning)', fontWeight: 600, fontSize: '0.85rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
    ...
    <div>
      ...
      {enCours.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 10 }}>
          {enCours.map(obj => (
            <div key={obj.id} style={{ background: 'var(--color-warning-bg)', border: '1px solid var(--color-warning-border)', borderRadius: 10, padding: '0.7rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 }}>
                <div>
                  <div style={{ fontWeight: 700, fontSize: '0.85rem', color: 'var(--color-warning-dark)' }}>{obj.titre}</div>
                  <div style={{ fontSize: '0.78rem', color: 'var(--color-amber)' }}>{obj.cible}</div>
                </div>
                {obj.dateCible && <span style={{ fontSize: '0.7rem', color: 'var(--color-warning)', fontWeight: 600, flexShrink: 0 }}>{obj.dateCible}</span>}
              </div>
              <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
                <button onClick={() => updateStatus(obj.id, 'atteint')}
                  style={{ flex: 1, padding: '0.4rem', borderRadius: 8, border: '1px solid var(--color-success-border)', background: 'var(--color-success-bg)', color: 'var(--color-success)', fontWeight: 700, fontSize: '0.75rem', cursor: 'pointer' }}>
                  Atteint
                </button>
                <button onClick={() => updateStatus(obj.id, 'non_atteint')}
                  style={{ flex: 1, padding: '0.4rem', borderRadius: 8, border: '1px solid var(--color-danger-border)', background: 'var(--color-danger-bg)', color: 'var(--color-danger)', fontWeight: 700, fontSize: '0.75rem', cursor: 'pointer' }}>
                  Non atteint
                </button>
      ...
      {/* Add form */}
      {showAdd ? (
        <div style={{ background: 'var(--surface)', border: '1.5px solid var(--color-warning-border)', borderRadius: 10, padding: '0.75rem', marginBottom: 6 }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <button onClick={handleAdd} disabled={!titre.trim() || !cible.trim()}
              style={{ flex: 1, padding: '0.55rem', borderRadius: 8, background: titre.trim() && cible.trim() ? 'linear-gradient(135deg, var(--color-warning), var(--color-amber))' : 'var(--secondary)', border: 'none', color: titre.trim() && cible.trim() ? 'white' : 'var(--text-muted)', fontWeight: 700, fontSize: '0.85rem', cursor: titre.trim() && cible.trim() ? 'pointer' : 'not-allowed' }}>
              Ajouter
            </button>
        ...
        <button onClick={() => setShowAdd(true)}
          style={{ width: '100%', padding: '0.55rem', borderRadius: 8, border: '1.5px dashed var(--color-warning-border)', background: 'transparent', color: 'var(--color-warning)', fontWeight: 600, fontSize: '0.82rem', cursor: 'pointer' }}>
          + Ajouter un objectif
        </button>
      )}
    </div>
