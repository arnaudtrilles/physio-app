  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
        <div style={{ width: 28, height: 28, borderRadius: 8, background: 'var(--color-warning-bg)', border: '1px solid var(--color-warning-border)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        ...
        <div style={{ fontSize: '0.82rem', fontWeight: 700, color: 'var(--color-warning-dark)' }}>Objectifs SMART</div>
      </div>
      ...
      {objectifs.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 10 }}>
          {objectifs.map(obj => (
            <div key={obj.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, padding: '0.55rem 0.7rem', background: 'var(--color-warning-bg)', border: '1px solid var(--color-warning-border)', borderRadius: 8 }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--color-warning)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ marginTop: 2, flexShrink: 0 }}>
                <polyline points="9 11 12 14 22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/>
              </svg>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 600, fontSize: '0.82rem', color: 'var(--color-warning-dark)' }}>{obj.titre}</div>
                {obj.cible && <div style={{ fontSize: '0.75rem', color: 'var(--color-amber)' }}>{obj.cible}</div>}
      ...
      {/* Formulaire d'ajout */}
      {showAdd ? (
        <div style={{ background: 'var(--surface)', border: '1.5px solid var(--color-warning-border)', borderRadius: 10, padding: '0.7rem', marginBottom: 6 }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <button onClick={handleAdd} disabled={!titre.trim()}
              style={{ flex: 1, padding: '0.5rem', borderRadius: 8, background: titre.trim() ? 'linear-gradient(135deg, var(--color-warning), var(--color-amber))' : 'var(--secondary)', border: 'none', color: titre.trim() ? 'white' : 'var(--text-muted)', fontWeight: 700, fontSize: '0.82rem', cursor: titre.trim() ? 'pointer' : 'not-allowed' }}>
              Ajouter
            </button>
        ...
        <button onClick={() => setShowAdd(true)}
          style={{ width: '100%', padding: '0.5rem', borderRadius: 8, border: '1.5px dashed var(--color-warning-border)', background: 'transparent', color: 'var(--color-warning)', fontWeight: 600, fontSize: '0.8rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}>
