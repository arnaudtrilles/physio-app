...
const TENDANCE_CONFIG = {
  amelioration: { label: 'Amélioration', color: 'var(--color-success)', bg: 'var(--color-success-bg)', border: 'var(--color-success-border2)', icon: '▲' },
  stationnaire:  { label: 'Stationnaire', color: 'var(--color-warning)', bg: 'var(--color-warning-bg)', border: 'var(--pain-scale-yellow)', icon: '=' },
  regression:    { label: 'Régression',   color: 'var(--color-danger)', bg: 'var(--color-danger-bg)', border: 'var(--color-danger-border)', icon: '▼' },
  mixte:         { label: 'Évolution mixte', color: 'var(--color-purple)', bg: 'var(--color-purple-bg)', border: 'var(--color-purple-border)', icon: '~' },
}
      ...
      <div className="scroll-area" style={{ paddingBottom: '5.5rem' }}>
        ...
        {!apiKey && (
          <div style={{ background: 'var(--color-warning-bg)', border: '1px solid var(--color-warning-border)', borderRadius: 14, padding: 20, marginBottom: 12 }}>
            <div style={{ fontWeight: 700, color: 'var(--color-warning-dark)', marginBottom: 6 }}>Clé API Gemini requise</div>
            <p style={{ fontSize: '0.85rem', color: 'var(--color-warning-deeper)', margin: '0 0 14px' }}>Configurez votre clé API Gemini dans votre profil.</p>
        ...
        {error === 'auth' && (
          <div style={{ background: 'var(--color-danger-bg)', border: '1px solid var(--color-danger-border)', borderRadius: 14, padding: 20, marginBottom: 12 }}>
            <div style={{ fontWeight: 700, color: 'var(--color-danger-dark)', marginBottom: 8 }}>Clé API Gemini invalide</div>
            <p style={{ fontSize: '0.85rem', color: 'var(--color-danger-deeper)', margin: '0 0 14px' }}>Renseignez une clé valide (AIza...) dans votre profil.</p>
            <button onClick={onGoToProfile}
              style={{ width: '100%', padding: '0.75rem', borderRadius: 10, background: 'linear-gradient(135deg, var(--color-purple-dark), var(--color-purple))', color: 'white', fontWeight: 700, fontSize: '0.9rem', border: 'none', cursor: 'pointer' }}>
        ...
        {error && error !== 'quota' && error !== 'auth' && (
          <div style={{ background: 'var(--color-danger-bg)', border: '1px solid var(--color-danger-border)', borderRadius: 12, padding: 16, marginBottom: 12 }}>
            ...
            <button onClick={() => { setRetryCount(0); runAnalysis(0) }} style={{ marginTop: 8, fontSize: '0.82rem', color: 'var(--color-purple)', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>Réessayer</button>
          </div>
        )}
          ...
          <div className="fade-in-up">
            ...
            <div className="ai-section-card">
              <div className="ai-section-header">
                ...
                <h4 style={{ color: 'var(--primary)' }}>Résumé clinique</h4>
              </div>
              <div className="ai-section-body">
            ...
            <div className="ai-section-card">
              <div className="ai-section-header">
                ...
                <h4 style={{ color: 'var(--color-success-dark)' }}>Progression séance par séance</h4>
              </div>
              <div className="ai-section-body">
                {evolution.progression.map((p, i) => {
                  ...
                  const dColor = delta === null ? 'var(--color-slate)' : delta > 0 ? 'var(--color-success)' : delta < 0 ? 'var(--color-danger)' : 'var(--color-slate)'
                  return (
                    <div key={i} style={{ display: 'flex', gap: 12, marginBottom: 14, alignItems: 'flex-start' }}>
                      <div style={{ width: 32, height: 32, borderRadius: 8, background: 'var(--color-slate-light)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '0.8rem', color: 'var(--primary-dark)', flexShrink: 0 }}>
                        {p.bilanNum}
                      </div>
                      <div style={{ flex: 1 }}>
            ...
            {evolution.pointsForts.length > 0 && (
              <div className="ai-section-card">
                <div className="ai-section-header">
                  ...
                  <h4 style={{ color: 'var(--color-success-dark)' }}>Points forts</h4>
                </div>
                <div className="ai-section-body">
            ...
            {evolution.pointsVigilance.length > 0 && (
              <div className="ai-section-card">
                <div className="ai-section-header">
                  ...
                  <h4 style={{ color: 'var(--color-orange-red)' }}>Points de vigilance</h4>
                </div>
                <div className="ai-section-body">
                  {evolution.pointsVigilance.map((p, i) => (
                    <div key={i} className="ai-alerte-item" style={{ marginBottom: 8 }}>
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--color-orange-dark)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, marginTop: 1 }}>
            ...
            {evolution.recommandations.length > 0 && (
              <div className="ai-section-card">
                <div className="ai-section-header">
                  <div className="ai-section-icon" style={{ background: 'var(--color-purple-bg)' }}>
                  ...
                  <h4 style={{ color: 'var(--color-purple-dark)' }}>Recommandations thérapeutiques</h4>
                </div>
                <div className="ai-section-body">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {!evolution && !loading && apiKey && !error && (
            <button className="btn-primary-luxe" style={{ marginBottom: 0, background: 'linear-gradient(135deg, var(--color-purple-dark), var(--color-purple))' }} onClick={() => runAnalysis(0)}>
              Générer le rapport d'évolution
            </button>
          )}
          {loading && (
            <button className="btn-primary-luxe" disabled style={{ marginBottom: 0, opacity: 0.7, background: 'linear-gradient(135deg, var(--color-purple-dark), var(--color-purple))', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
