...
const EVOLUTION_COLORS: Record<string, string> = {
  'Amélioré': 'var(--color-success)',
  'Stable': 'var(--color-zinc)',
  'Aggravé': 'var(--color-danger)',
}
  ...
  return (
    <div style={cardStyle}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
        <span style={{ fontWeight: 700, fontSize: 14, color: 'var(--primary)' }}>Bilan initial</span>
        {hasIA && <Badge label="IA" bg="#dbeafe" fg="var(--primary)" />}
        {incomplet && <Badge label="Incomplet" bg="var(--color-amber-bg)" fg="var(--color-warning-dark)" />}
      </div>
...
function IntermediaireCard({ item }: { item: IntermediaireItem }) {
  ...
  return (
    <div style={cardStyle}>
      <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--color-orange)' }}>
        Bilan intermédiaire
      </div>
      <div style={metaRowStyle}>
        <span>{item.dateBilan}</span>
...
function NoteCard({ item }: { item: NoteSeanceItem }) {
  const evoBg = EVOLUTION_COLORS[item.data.evolution] ?? 'var(--color-zinc)'

  return (
    <div style={cardStyle}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
        <span style={{ fontWeight: 700, fontSize: 14, color: 'var(--color-purple-dark)' }}>
