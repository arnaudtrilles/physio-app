...
import type { BilanRecord, BilanIntermediaireRecord, NoteSeanceRecord, SmartObjectif, ProfileData, AnalyseIA, FicheExercice, BilanDocument, Subscription } from './types'
...
import { AgentVocalIA } from './components/AgentVocalIA'
import { mapAgentField } from './utils/agentFieldMapper'
import './App.css'

const ELEVENLABS_AGENT_ID = 'agent_3301knadhqjfemtsk85mqjctwzw9'

type Step = 'welcome' | 'dashboard' | 'database' | 'profile' | 'identity' | 'general_info' | 'silhouette' | 'bilan_zone' | 'bilan_intermediaire' | 'note_intermediaire' | 'note_seance' | 'pdf_preview' | 'analyse_ia' | 'evolution_ia' | 'fiche_exercice'
...
const DEMO_DB: BilanRecord[] = [
  ...
  { id:6,  nom:'MARCHAND', prenom:'Sophie', dateNaissance:'03/08/1990', dateBilan:'01/10/2025', zoneCount:1, evn:7, zone:'Genou Droit',   pathologie:'Syndrome fémoro-patellaire',           avatarBg:'var(--color-purple-deeper)', bilanType:'genou',  status:'complet' },
  { id:7,  nom:'MARCHAND', prenom:'Sophie', dateNaissance:'03/08/1990', dateBilan:'15/10/2025', zoneCount:1, evn:5, zone:'Genou Droit',   pathologie:'Syndrome fémoro-patellaire',           avatarBg:'var(--color-purple-deeper)', bilanType:'genou',  status:'complet' },
  { id:8,  nom:'MARCHAND', prenom:'Sophie', dateNaissance:'03/08/1990', dateBilan:'29/10/2025', zoneCount:1, evn:6, zone:'Genou Droit',   pathologie:'Syndrome fémoro-patellaire',           avatarBg:'var(--color-purple-deeper)', bilanType:'genou',  status:'complet' },
  { id:9,  nom:'MARCHAND', prenom:'Sophie', dateNaissance:'03/08/1990', dateBilan:'12/11/2025', zoneCount:1, evn:4, zone:'Genou Droit',   pathologie:'Syndrome fémoro-patellaire',           avatarBg:'var(--color-purple-deeper)', bilanType:'genou',  status:'complet' },
  { id:10, nom:'MARCHAND', prenom:'Sophie', dateNaissance:'03/08/1990', dateBilan:'26/11/2025', zoneCount:1, evn:3, zone:'Genou Droit',   pathologie:'Syndrome fémoro-patellaire',           avatarBg:'var(--color-purple-deeper)', bilanType:'genou',  status:'complet' },
  { id:11, nom:'MARCHAND', prenom:'Sophie', dateNaissance:'03/08/1990', dateBilan:'10/12/2025', zoneCount:1, evn:2, zone:'Genou Droit',   pathologie:'Syndrome fémoro-patellaire',           avatarBg:'var(--color-purple-deeper)', bilanType:'genou',  status:'complet' },
  { id:12, nom:'PETIT',    prenom:'Lucas',  dateNaissance:'22/11/1975', dateBilan:'05/11/2025', zoneCount:2, evn:9, zone:'Rachis Lombaire',pathologie:'Lombalgie chronique commune',          avatarBg:'var(--color-orange-mid)', bilanType:'lombaire',status:'complet' },
  { id:13, nom:'PETIT',    prenom:'Lucas',  dateNaissance:'22/11/1975', dateBilan:'19/11/2025', zoneCount:2, evn:7, zone:'Rachis Lombaire',pathologie:'Lombalgie chronique commune',          avatarBg:'var(--color-orange-mid)', bilanType:'lombaire',status:'complet' },
  { id:14, nom:'PETIT',    prenom:'Lucas',  dateNaissance:'22/11/1975', dateBilan:'03/12/2025', zoneCount:2, evn:8, zone:'Rachis Lombaire',pathologie:'Lombalgie chronique commune',          avatarBg:'var(--color-orange-mid)', bilanType:'lombaire',status:'complet' },
  { id:15, nom:'PETIT',    prenom:'Lucas',  dateNaissance:'22/11/1975', dateBilan:'17/12/2025', zoneCount:2, evn:6, zone:'Rachis Lombaire',pathologie:'Lombalgie chronique commune',          avatarBg:'var(--color-orange-mid)', bilanType:'lombaire',status:'complet' },
]
...
function App() {
  ...
  const [subscription, setSubscription] = useLocalStorage<Subscription | null>('physio_subscription', null)
  const [showAgentVocal, setShowAgentVocal] = useState(false)

  // TODO: réactiver le gate d'abonnement à la commercialisation
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const _subscription = subscription
  const hasAgentAccess = true
  ...
  const [savedBtn, setSavedBtn] = useState<string | null>(null)
  const withSavedFeedback = (key: string, fn: () => void, delay = 700) => {
    setSavedBtn(key)
    fn()
    setTimeout(() => setSavedBtn(null), delay)
  }
  ...
  const resetForm = () => {
    setFormData({ nom: '', prenom: '', dateNaissance: '', profession: '', sport: '', famille: '', chirurgie: '', notes: '', telephone: '' })
    setSilhouetteData({})
    setBilanDocuments([])
    setPatientMode('new')
...
### 5. Plan de Traitement et Démarche Thérapeutique`
  ...
  const exportBilanFromRecord = async (record: BilanRecord, isIntermediaire = false) => {
    setFormData(prev => ({ ...prev, nom: record.nom, prenom: record.prenom, dateNaissance: record.dateNaissance, telephone: record.telephone ?? '' }))
    ...
    return (
      <div className="app-container" style={{ padding: 0 }}>
        <WelcomeScreen
          ...
          onSignup={(sub) => { setSubscription(sub); setStep('dashboard') }}
        />
      </div>
    )
  }
    ...
    <div className="app-container">
      ...
      {step === 'dashboard' && (
        <div className="start-screen fade-in">
          ...
          <div style={{ width: '100%', marginBottom: '5.5rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            ...
            {/* Agent Vocal IA — Pro Max */}
            {hasAgentAccess ? (
              <button
                onClick={() => setShowAgentVocal(true)}
                style={{
              ...
              >
                ...
                Agent IA Vocal
                {subscription?.trialAgentUntil && (
                  <span style={{ fontSize: '0.7rem', background: 'rgba(255,255,255,0.2)', borderRadius: '999px', padding: '2px 8px' }}>
                    Essai {Math.max(0, Math.ceil((new Date(subscription.trialAgentUntil).getTime() - Date.now()) / 86400000))}j
                  </span>
                )}
              </button>
            ) : (
              <button
                onClick={() => showToast('Disponible avec le forfait Multi Pro Max', 'info')}
                style={{
                  width: '100%', padding: '0.85rem',
                  borderRadius: 'var(--radius-lg)',
                  background: 'rgba(124,58,237,0.08)',
                  border: '1.5px dashed rgba(124,58,237,0.3)',
                  color: 'var(--color-purple)', fontSize: '0.88rem', fontWeight: 600,
      ...
      {/* Agent Vocal IA overlay */}
      {showAgentVocal && (
        <AgentVocalIA
          agentId={ELEVENLABS_AGENT_ID}
          onClose={() => setShowAgentVocal(false)}
          toolCallbacks={{
            db,
            therapistName: `${profile.prenom} ${profile.nom}`.trim() || 'Thérapeute',
            onCreatePatient: (nom, prenom, dateNaissance) => {
              const id = Date.now()
              const newRecord: BilanRecord = {
                id, nom: nom.toUpperCase(), prenom,
                dateNaissance, dateBilan: new Date().toLocaleDateString('fr-FR'),
                zoneCount: 0, status: 'incomplet',
              }
              setDb(prev => [newRecord, ...prev])
              return id
            },
            onSetBilanZone: (zone, _typeBilan) => {
              resetForm()
            ...
            onFillField: (field, value) => {
              const bilanType = getBilanType(selectedBodyZone ?? '')
              const data = mapAgentField(bilanType, field, value)
              if (!data) return
              switch (bilanType) {
                case 'epaule':   bilanEpauleRef.current?.setData(data);   break
                case 'cheville': bilanChevilleRef.current?.setData(data); break
                case 'genou':    bilanGenouRef.current?.setData(data);    break
                case 'hanche':   bilanHancheRef.current?.setData(data);   break
                case 'cervical': bilanCervicalRef.current?.setData(data); break
                case 'lombaire': bilanLombaireRef.current?.setData(data); break
                default:         bilanGeneriqueRef.current?.setData(data)
              }
            },
            onSaveBilan: () => {
              saveBilan('complet')
            ...
            onGenerateAnalyseIA: () => {
              handleSaveAndAnalyse()
            },
            onExportPDF: () => {
              handleExportPDF()
            },
            onGenerateFicheExercice: () => {
              setFicheBackStep('analyse_ia')
      ...
      {step === 'database' && (
        <div className="general-info-screen fade-in">
          <header className="screen-header">
            {selectedPatient
              ? <button className="btn-back" aria-label="Retour" onClick={() => setSelectedPatient(null)}>
                  ...
                  return (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', paddingBottom: '5rem' }}>
                      {patients.map(p => {
                        ...
                        const scoreColor = score === null ? 'var(--color-slate)' : score > 0 ? 'var(--color-success)' : 'var(--color-danger)'
                        const scoreBg   = score === null ? 'var(--color-slate-light)' : score > 0 ? 'var(--color-success-bg2)' : 'var(--color-danger-bg2)'
                        ...
                        return (
                          <div key={p.key} onClick={() => setSelectedPatient(p.key)}
                            ...
                            {score !== null
                              ...
                              : <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--color-slate)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"/></svg>
                            }
                          </div>
                        )
                      })}
                    ...
                    <>
                      ...
                      <div style={{ marginBottom: '0.75rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.2rem 0.5rem', marginBottom: '0.5rem' }}>
                          <div style={{ flex: 1, height: 1, background: 'var(--color-warning-border)' }} />
                          <span style={{ fontSize: '0.65rem', color: 'var(--color-warning)', fontWeight: 700, letterSpacing: '0.05em' }}>OBJECTIFS SMART</span>
                          <div style={{ flex: 1, height: 1, background: 'var(--color-warning-border)' }} />
                        </div>
                        ...
                        return (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '1.25rem' }}>
                            {zoneScores.map(item => {
                              ...
                              const color  = score > 0 ? 'var(--color-success)' : score < 0 ? 'var(--color-danger)' : 'var(--color-slate)'
                              const bg     = score > 0 ? 'var(--color-success-bg)' : score < 0 ? 'var(--color-danger-bg)' : 'var(--color-slate-light)'
                              const border = score > 0 ? 'var(--color-success-border2)' : score < 0 ? 'var(--color-danger-border)' : 'var(--base-bg)'
                              return (
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem', paddingBottom: '5rem' }}>
                        {(() => {
                          ...
                          const dColor  = delta === null ? '' : delta > 0 ? 'var(--color-success)' : delta < 0 ? 'var(--color-danger)' : 'var(--color-slate)'
                          const dBg     = delta === null ? '' : delta > 0 ? 'var(--color-success-bg2)' : delta < 0 ? 'var(--color-danger-bg2)' : 'var(--color-slate-light)'
                          ...
                          return (
                            <div key={record.id} style={{ background: 'var(--surface)', padding: '0.9rem 1.25rem', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-sm)' }}>
                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                <div>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.15rem' }}>
                                    ...
                                    {!incomplet && record.analyseIA && (
                                      <span style={{ fontSize: '0.7rem', fontWeight: 600, padding: '0.1rem 0.5rem', borderRadius: 'var(--radius-full)', background: 'var(--color-info-bg)', color: 'var(--color-info)', border: '1px solid var(--color-info-border)' }}>IA</span>
                                    )}
                                  </div>
                                ...
                                {incomplet ? (
                                  <span style={{ fontSize: '0.72rem', fontWeight: 700, color: 'var(--color-danger-dark)', flexShrink: 0, paddingTop: 2 }}>Incomplet</span>
                                ) : delta !== null ? (
                              ...
                              {incomplet ? (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: '0.75rem' }}>
                                  ...
                                  <div style={{ display: 'flex', justifyContent: 'center', gap: 16, paddingTop: 2 }}>
                                    <button onClick={() => setDeletingBilanId(record.id)}
                                      style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.78rem', color: 'var(--color-danger)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4 }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: '0.75rem' }}>
                                  ...
                                  <div style={{ display: 'flex', gap: 6 }}>
                                    ...
                                    <button
                                      style={{ flex: 1, padding: '0.6rem 0.5rem', borderRadius: 10, background: 'var(--color-info-bg)', border: '1.5px solid var(--color-info-border)', color: 'var(--color-info)', fontWeight: 700, fontSize: '0.82rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}
                                      onClick={() => {
                                        setFormData(prev => ({ ...prev, nom: record.nom, prenom: record.prenom, dateNaissance: record.dateNaissance, telephone: record.telephone ?? '' }))
                                        setSelectedBodyZone(record.zone ?? null)
                                        setCurrentBilanId(record.id)
                                  ...
                                  <button
                                    style={{ width: '100%', padding: '0.6rem 1rem', borderRadius: 10, background: 'var(--color-success-bg)', border: '1.5px solid var(--color-success-border)', color: 'var(--color-success-deeper)', fontWeight: 700, fontSize: '0.82rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
                                    onClick={() => {
                                      setFormData(prev => ({ ...prev, nom: record.nom, prenom: record.prenom, dateNaissance: record.dateNaissance, telephone: record.telephone ?? '' }))
                                      setSelectedBodyZone(record.zone ?? null)
                                      setCurrentBilanId(record.id)
                                ...
                                return (
                                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginTop: '0.5rem' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.2rem 0.5rem' }}>
                                      <div style={{ flex: 1, height: 1, background: 'var(--color-orange-border)' }} />
                                      <span style={{ fontSize: '0.65rem', color: 'var(--color-orange-dark)', fontWeight: 700, letterSpacing: '0.05em' }}>BILANS INTERMÉDIAIRES</span>
                                      <div style={{ flex: 1, height: 1, background: 'var(--color-orange-border)' }} />
                                    </div>
                                    {inters.map((rec, idx) => {
                                      ...
                                      const sColor = score === null ? 'var(--color-slate)' : score > 0 ? 'var(--color-success)' : score < 0 ? 'var(--color-danger)' : 'var(--color-slate)'
                                      const sBg    = score === null ? '' : score > 0 ? 'var(--color-success-bg2)' : score < 0 ? 'var(--color-danger-bg2)' : 'var(--color-slate-light)'
                                      return (
                                      <div key={rec.id} style={{ background: 'var(--color-orange-bg)', border: '1px solid var(--color-orange-border)', padding: '0.85rem 1.25rem', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-sm)' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.6rem' }}>
                                          <div>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.1rem' }}>
                                              <span style={{ fontWeight: 600, color: 'var(--color-warning-dark)', fontSize: '0.92rem' }}>Intermédiaire N°{idx + 1}</span>
                                              {rec.status === 'incomplet' && <span style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--color-danger-dark)' }}>Incomplet</span>}
                                            </div>
                                            <div style={{ fontSize: '0.78rem', color: 'var(--color-orange-dark)' }}>{rec.dateBilan}</div>
                                          </div>
                                          {score !== null && (
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                                          {rec.status === 'incomplet' ? (
                                            <>
                                              <button
                                                style={{ width: '100%', padding: '0.55rem 1rem', borderRadius: 10, background: 'linear-gradient(135deg, var(--color-orange), var(--color-orange-dark))', border: 'none', color: 'white', fontWeight: 700, fontSize: '0.82rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
                                                onClick={() => {
                                                  setFormData(prev => ({ ...prev, nom: rec.nom, prenom: rec.prenom, dateNaissance: rec.dateNaissance }))
                                              ...
                                              <div style={{ display: 'flex', justifyContent: 'center', gap: 16, paddingTop: 2 }}>
                                                <button onClick={() => setDeletingIntermediaireId(rec.id)}
                                                  style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.75rem', color: 'var(--color-danger)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4 }}>
                                            ...
                                            <>
                                              ...
                                              <div style={{ display: 'flex', gap: 6 }}>
                                                ...
                                                <button
                                                  style={{ flex: 1, padding: '0.55rem 0.5rem', borderRadius: 10, background: 'var(--color-orange-bg)', border: '1.5px solid var(--color-orange-border)', color: 'var(--color-warning-dark)', fontWeight: 700, fontSize: '0.78rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}
                                              ...
                                              {/* Rangée 2 : Fiche d'exercices */}
                                              <button
                                                style={{ width: '100%', padding: '0.55rem 1rem', borderRadius: 10, background: 'var(--color-success-bg)', border: '1.5px solid var(--color-success-border)', color: 'var(--color-success-deeper)', fontWeight: 700, fontSize: '0.82rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
                                                onClick={() => {
                                                  setFormData(prev => ({ ...prev, nom: rec.nom, prenom: rec.prenom, dateNaissance: rec.dateNaissance }))
                                                  setFicheBackStep('database')
                                                  setStep('fiche_exercice')
                        ...
                        {bilans.filter(r => r.status === 'complet').length >= 2 && (
                          <button
                            ...
                            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, width: '100%', padding: '0.85rem', borderRadius: 'var(--radius-lg)', background: 'linear-gradient(135deg, var(--color-purple-dark), var(--color-purple))', border: 'none', color: 'white', fontWeight: 700, fontSize: '0.9rem', cursor: 'pointer', boxShadow: '0 2px 8px rgba(124,58,237,0.3)' }}>
                        ...
                        {(() => {
                          ...
                          return (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginTop: '0.25rem' }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.2rem 0.5rem' }}>
                                <div style={{ flex: 1, height: 1, background: 'var(--color-purple-border)' }} />
                                <span style={{ fontSize: '0.65rem', color: 'var(--color-purple-dark)', fontWeight: 700, letterSpacing: '0.05em' }}>NOTES DE SÉANCE</span>
                                <div style={{ flex: 1, height: 1, background: 'var(--color-purple-border)' }} />
                              </div>
                              {notes.map(note => {
                                ...
                                return (
                                  <div key={note.id} style={{ background: 'var(--surface)', borderRadius: 12, border: '1.5px solid var(--color-purple-light)', padding: '0.75rem', boxShadow: '0 1px 3px rgba(109,40,217,0.06)' }}>
                                    {/* Header */}
                                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                                      <div>
                                        <span style={{ fontWeight: 700, fontSize: '0.85rem', color: 'var(--color-purple-deeper2)' }}>Séance n°{note.numSeance}</span>
                                        <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginLeft: 6 }}>{ZONE_LABELS[zt] ?? zt}</span>
                                      </div>
                                      <div style={{ fontSize: '0.78rem', color: 'var(--color-purple-dark)' }}>{note.dateSeance}</div>
                                    </div>
                                    {/* Badges */}
                                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 6 }}>
                                      {note.data.eva && <span style={{ fontSize: '0.7rem', fontWeight: 600, padding: '0.1rem 0.45rem', borderRadius: 'var(--radius-full)', background: 'var(--color-violet-light)', color: 'var(--color-purple-dark)' }}>EVA {note.data.eva}/10</span>}
                                      {note.data.evolution && <span style={{ fontSize: '0.7rem', fontWeight: 600, padding: '0.1rem 0.45rem', borderRadius: 'var(--radius-full)', background: note.data.evolution === 'Amélioré' ? 'var(--color-success-bg2)' : note.data.evolution === 'Aggravé' ? 'var(--color-danger-bg)' : 'var(--color-gray-light)', color: note.data.evolution === 'Amélioré' ? 'var(--color-success)' : note.data.evolution === 'Aggravé' ? 'var(--color-danger)' : 'var(--color-gray-mid)' }}>{note.data.evolution}</span>}
                                      {note.data.tolerance && <span style={{ fontSize: '0.7rem', fontWeight: 600, padding: '0.1rem 0.45rem', borderRadius: 'var(--radius-full)', background: note.data.tolerance === 'Bien toléré' ? 'var(--color-success-bg2)' : 'var(--color-warning-bg)', color: note.data.tolerance === 'Bien toléré' ? 'var(--color-success)' : 'var(--color-warning)' }}>{note.data.tolerance}</span>}
                                    </div>
                                    {/* Preview contenu */}
                                    <div style={{ background: 'var(--color-purple-bg)', borderRadius: 8, padding: '0.55rem 0.7rem', marginBottom: 8, fontSize: '0.75rem', color: 'var(--color-purple-deeper2)', lineHeight: 1.5 }}>
                                    ...
                                    {note.analyseIA && (
                                      <div style={{ marginBottom: 8 }}>
                                        <button onClick={() => {
                                          ...
                                          style={{ width: '100%', padding: '0.4rem 0.7rem', borderRadius: 8, background: 'var(--color-info-bg)', border: '1px solid var(--color-info-border)', color: 'var(--color-info)', fontWeight: 600, fontSize: '0.72rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                        ...
                                        <div id={`analyse-seance-${note.id}`} style={{ background: 'var(--color-info-bg)', borderRadius: '0 0 8px 8px', padding: '0.55rem 0.7rem', fontSize: '0.75rem', color: 'var(--primary)', lineHeight: 1.5, borderLeft: '3px solid var(--accent)', borderTop: 'none' }}>
                                          ...
                                          {note.analyseIA.vigilance.length > 0 && (
                                            <div style={{ color: 'var(--color-danger)', fontSize: '0.72rem', marginBottom: 3 }}>Vigilance : {note.analyseIA.vigilance.join(' / ')}</div>
                                          )}
                                          <div style={{ fontWeight: 600, fontSize: '0.72rem' }}>Focus : {note.analyseIA.focus}</div>
                                          {note.analyseIA.conseil && (
                                            <div style={{ marginTop: 4, padding: '0.4rem 0.55rem', background: 'var(--color-success-bg)', borderRadius: 6, border: '1px solid var(--color-success-border)', color: 'var(--color-success-deeper)', fontSize: '0.72rem' }}>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                                      <div style={{ display: 'flex', gap: 6 }}>
                                        <button
                                          style={{ flex: 1, padding: '0.5rem 0.5rem', borderRadius: 10, background: 'var(--color-info-bg)', border: '1.5px solid var(--color-info-border)', color: 'var(--color-info)', fontWeight: 700, fontSize: '0.78rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}
                                          onClick={async () => {
                                        ...
                                        <button
                                          style={{ flex: 1, padding: '0.5rem 0.5rem', borderRadius: 10, background: 'var(--color-success-bg)', border: '1.5px solid var(--color-success-border)', color: 'var(--color-success-deeper)', fontWeight: 700, fontSize: '0.78rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}
                                          onClick={() => {
                        ...
                        <button
                          ...
                          style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', width: '100%', padding: '0.75rem', borderRadius: 'var(--radius-lg)', border: '2px dashed var(--color-purple-border)', background: 'transparent', color: 'var(--color-purple-dark)', fontWeight: 600, fontSize: '0.88rem', cursor: 'pointer' }}>
                        ...
                        <div style={{ display: 'flex', gap: 8 }}>
                          <button
                            ...
                            style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.35rem', padding: '0.7rem 0.5rem', borderRadius: 'var(--radius-lg)', border: '2px dashed var(--color-orange-border)', background: 'transparent', color: 'var(--color-orange-dark)', fontWeight: 600, fontSize: '0.78rem', cursor: 'pointer' }}>
      ...
      {step === 'profile' && (() => {
        ...
        const gsColor = globalScore >= 50 ? 'var(--color-success)' : globalScore >= 20 ? 'var(--color-orange-mid)' : 'var(--color-danger)'
        const incompletCount = db.filter(r => r.status === 'incomplet').length
        return (
          <div className="general-info-screen fade-in">
            <header className="screen-header">
              <button className="btn-back" aria-label="Retour" onClick={() => editingProfile ? setEditingProfile(false) : setStep('dashboard')}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
              </button>
              ...
              {editingProfile ? (
                <div className="fade-in">
                  ...
                  <div style={{ marginTop: '1.5rem', marginBottom: '1rem' }}>
                    ...
                    <div style={{ background: 'var(--secondary)', borderRadius: 12, padding: 16, border: '1px solid var(--border-color)' }}>
                      ...
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                        ...
                        {apiKeyStatus === 'ok'  && <span style={{ fontSize: '0.8rem', color: 'var(--color-success)', fontWeight: 600 }}>Connecté</span>}
                        {apiKeyStatus === 'error'&& <span style={{ fontSize: '0.8rem', color: 'var(--color-danger)', fontWeight: 600 }}>Invalide</span>}
                      </div>
                ...
                <>
                  <div style={{ display:'flex', alignItems:'center', gap:'1rem', marginBottom:'1.5rem', padding:'1rem', background:'var(--secondary)', borderRadius:'var(--radius-lg)' }}>
                    ...
                    <div>
                      ...
                      {apiKey && <div style={{ fontSize:'0.75rem', color:'var(--color-success)', fontWeight:600, marginTop:2 }}>IA active</div>}
                    </div>
                    {!isOnline && (
                      <span style={{ fontSize: '0.7rem', fontWeight: 700, padding: '0.15rem 0.5rem', borderRadius: 'var(--radius-full)', background: 'var(--color-danger-bg)', color: 'var(--color-danger)', border: '1px solid var(--color-danger-border)' }}>
                        Hors-ligne
                      </span>
                    )}
                  </div>
                  ...
                  <div style={{ background:'var(--surface)', borderRadius:'var(--radius-xl)', padding:'1.25rem', marginBottom:'1.25rem', boxShadow:'var(--shadow-sm)', border:'1px solid var(--border-color)' }}>
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:'1.25rem' }}>
                      <svg viewBox="0 0 150 150" width="130" height="130" style={{ flexShrink:0 }}>
                        <circle cx="75" cy="75" r={r} fill="none" stroke="var(--base-bg)" strokeWidth="14"/>
                        {forte   > 0 && <circle cx="75" cy="75" r={r} fill="none" stroke="var(--color-success)" strokeWidth="14" strokeDasharray={`${seg(forte)} ${circ}`}   strokeDashoffset={startOff} strokeLinecap="round"/>}
                        {moderee > 0 && <circle cx="75" cy="75" r={r} fill="none" stroke="var(--color-orange-mid)" strokeWidth="14" strokeDasharray={`${seg(moderee)} ${circ}`} strokeDashoffset={startOff - slot(forte)} strokeLinecap="round"/>}
                        {regressN> 0 && <circle cx="75" cy="75" r={r} fill="none" stroke="var(--color-danger)" strokeWidth="14" strokeDasharray={`${seg(regressN)} ${circ}`} strokeDashoffset={startOff - slot(forte) - slot(moderee)} strokeLinecap="round"/>}
                        <text x="75" y="69" textAnchor="middle" fill="var(--primary-dark)" fontSize="26" fontWeight="800">{total}</text>
                        <text x="75" y="87" textAnchor="middle" fill="var(--color-slate)" fontSize="11">patients</text>
                      </svg>
                      <div style={{ flex:1, display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0.85rem' }}>
                        {[
                          ...
                          { label:'En amélioration', value: forte + moderee, color:'var(--color-success)' },
                          { label:'En régression', value: regressN, color:'var(--color-danger)' },
                          { label:'Bilans incomplets', value: incompletCount, color: incompletCount > 0 ? 'var(--color-warning)' : 'var(--color-slate)' },
                        ].map(s => (
                          <div key={s.label}>
                    ...
                    <div style={{ display:'flex', gap:'1rem', flexWrap:'wrap', marginTop:'1rem', paddingTop:'0.75rem', borderTop:'1px solid var(--border-color)' }}>
                      {[{ c:'var(--color-success)', l:'Forte amélioration (>50%)' }, { c:'var(--color-orange-mid)', l:'Modérée (1–50%)' }, { c:'var(--color-danger)', l:'Régression' }].map(({ c, l }) => (
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:'0.6rem' }}>
                    {allPatientKeys.map(key => {
                      ...
                      const sColor = score === null ? 'var(--color-slate)' : score > 0 ? 'var(--color-success)' : 'var(--color-danger)'
                      const sBg    = score === null ? 'var(--color-slate-light)' : score > 0 ? 'var(--color-success-bg2)' : 'var(--color-danger-bg2)'
      ...
      {deletingBilanId !== null && (
        <div role="dialog" aria-modal="true" aria-labelledby="dlg-bilan-title" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 3000, padding: '1.5rem' }}>
          <div style={{ background: 'var(--surface)', padding: '1.5rem', borderRadius: 'var(--radius-xl)', width: '100%', maxWidth: '360px', boxShadow: 'var(--shadow-2xl)' }}>
            <div id="dlg-bilan-title" style={{ fontWeight: 700, color: 'var(--color-danger)', fontSize: '1rem', marginBottom: '0.5rem' }}>Supprimer ce bilan ?</div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button autoFocus onClick={() => setDeletingBilanId(null)}
              ...
              <button onClick={() => {
                ...
                style={{ flex: 1, padding: '0.7rem', borderRadius: 10, background: 'var(--color-danger)', border: 'none', color: 'white', fontWeight: 700, fontSize: '0.9rem', cursor: 'pointer' }}>
                Supprimer
              </button>
            </div>
          </div>
      ...
      {deletingIntermediaireId !== null && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 3000, padding: '1.5rem' }}>
          <div style={{ background: 'var(--surface)', padding: '1.5rem', borderRadius: 'var(--radius-xl)', width: '100%', maxWidth: '360px', boxShadow: 'var(--shadow-2xl)' }}>
            <div style={{ fontWeight: 700, color: 'var(--color-orange-dark)', fontSize: '1rem', marginBottom: '0.5rem' }}>Supprimer ce bilan intermédiaire ?</div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={() => {
                ...
                style={{ flex: 1, padding: '0.7rem', borderRadius: 10, background: 'var(--color-orange-dark)', border: 'none', color: 'white', fontWeight: 700, fontSize: '0.9rem', cursor: 'pointer' }}>
                Supprimer
              </button>
            </div>
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, paddingBottom: '1rem' }}>
              {[
                ...
                <button key={zone} onClick={() => {
                  ...
                  setStep('bilan_intermediaire')
                }} style={{ padding: '0.85rem 1.25rem', borderRadius: 'var(--radius-lg)', background: 'var(--color-orange-bg)', border: '1.5px solid var(--color-orange-border)', color: 'var(--color-warning-dark)', fontWeight: 600, fontSize: '0.95rem', cursor: 'pointer', textAlign: 'left' }}>
                  {label}
                </button>
              ))}
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, paddingBottom: '1rem' }}>
              {[
                ...
                <button key={zone} onClick={() => {
                  ...
                  setStep('note_seance')
                }} style={{ padding: '0.85rem 1.25rem', borderRadius: 'var(--radius-lg)', background: 'var(--color-purple-bg)', border: '1.5px solid var(--color-purple-light)', color: 'var(--color-purple-deeper2)', fontWeight: 600, fontSize: '0.95rem', cursor: 'pointer', textAlign: 'left' }}>
                  {label}
                </button>
              ))}
            </div>
      ...
      {deletingNoteSeanceId !== null && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 3000, padding: '1.5rem' }}>
          <div style={{ background: 'var(--surface)', padding: '1.5rem', borderRadius: 'var(--radius-xl)', width: '100%', maxWidth: '360px', boxShadow: 'var(--shadow-2xl)' }}>
            <div style={{ fontWeight: 700, color: 'var(--color-danger)', fontSize: '1rem', marginBottom: '0.5rem' }}>Supprimer cette note ?</div>
            <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', margin: '0 0 1.25rem', lineHeight: 1.5 }}>
      ...
      {step === 'identity' && (
        <div className="identity-screen fade-in">
          <header className="screen-header">
            <button className="btn-back" aria-label="Retour" onClick={() => setStep('dashboard')}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
            </button>
            ...
            {patientMode === 'new' && (
              <>
                ...
                <div className="form-group">
                  ...
                  <input type="tel" className="input-luxe" placeholder="Ex: +33 6 12 34 56 78" value={formData.telephone} onChange={(e) => updateField('telephone', e.target.value)} />
                </div>
              </>
            )}
      ...
      {step === 'general_info' && (
        <div className="general-info-screen fade-in">
          <header className="screen-header">
            <button className="btn-back" aria-label="Retour" onClick={() => setStep('identity')}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
            </button>
      ...
      {step === 'silhouette' && (
        <div className="silhouette-screen fade-in">
          <header className="screen-header">
            <button className="btn-back" aria-label="Retour" onClick={() => setStep('general_info')}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
            </button>
          ...
          <div className="fixed-bottom" style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            ...
            <button className="btn-primary-luxe" style={{background: savedBtn === 'brouillon' ? 'var(--color-success)' : 'var(--primary-dark)', marginBottom: 0, transition: 'background 0.3s'}}
              onClick={() => withSavedFeedback('brouillon', () => { saveBilan('incomplet'); setTimeout(goToPatientRecord, 600) })}>
              {savedBtn === 'brouillon' ? '✓ Sauvegardé !' : 'Enregistrer brouillon'}
            </button>
          </div>
        </div>
      )}
      ...
      {selectedBodyZone && (
        <div className="general-info-screen" style={{ display: step === 'bilan_zone' ? 'flex' : 'none', flexDirection: 'column' }}>
          <header className="screen-header">
            <button className="btn-back" aria-label="Retour" onClick={() => { if (bilanZoneBackStep === 'database') { setCurrentBilanId(null); setCurrentBilanDataOverride(null) } setStep(bilanZoneBackStep) }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
            </button>
            <div style={{ flex: 1 }}>
          ...
          <div className="fixed-bottom" style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            <button className="btn-primary-luxe" style={{ marginBottom: 0, background: 'linear-gradient(135deg, var(--primary), var(--accent))' }}
            ...
            <button className="btn-primary-luxe"
              style={{ marginBottom: 0, background: savedBtn === 'sans-ia' ? 'var(--color-success)' : 'var(--primary-dark)', transition: 'background 0.3s' }}
              onClick={() => withSavedFeedback('sans-ia', () => { saveBilan('complet'); setCurrentBilanId(null); setCurrentBilanDataOverride(null); setBilanZoneBackStep('general_info'); setTimeout(goToPatientRecord, 600) })}>
              {savedBtn === 'sans-ia' ? '✓ Enregistré !' : 'Enregistrer sans IA'}
            </button>
            <button className="btn-primary-luxe"
              style={{ marginBottom: 0, background: 'linear-gradient(to right, var(--color-emerald), var(--color-emerald-dark))', opacity: exportingPDF ? 0.7 : 1 }}
        ...
        return (
          <div style={{ minHeight: '100vh', background: 'var(--background)', paddingBottom: '6rem' }}>
            <div style={{ position: 'sticky', top: 0, zIndex: 100, background: 'var(--surface)', borderBottom: '1px solid var(--border-color)', padding: '0.75rem 1rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 700, fontSize: '0.95rem', color: 'var(--color-purple-deeper2)' }}>Note de séance n°{numSeance}</div>
                <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{formData.nom} {formData.prenom} · {ZONE_LABELS[bilanType]}</div>
              </div>
              <span style={{ fontSize: '0.7rem', fontWeight: 700, padding: '0.2rem 0.6rem', borderRadius: 'var(--radius-full)', background: 'var(--color-purple-bg)', color: 'var(--color-purple-dark)', border: '1px solid var(--color-purple-light)' }}>SOAP</span>
            </div>
            ...
            <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: 'var(--surface)', borderTop: '1px solid var(--border-color)', padding: '0.75rem 1rem', display: 'flex', flexDirection: 'column', gap: 8, zIndex: 100 }}>
              <button
                style={{ width: '100%', padding: '0.85rem', borderRadius: 'var(--radius-lg)', background: savedBtn === 'note' ? 'var(--color-success)' : 'linear-gradient(135deg, var(--color-purple-dark), var(--color-purple))', border: 'none', color: 'white', fontWeight: 700, fontSize: '0.95rem', cursor: 'pointer', transition: 'background 0.3s' }}
                onClick={() => withSavedFeedback('note', handleSaveNote)}>
                {savedBtn === 'note' ? '✓ Note enregistrée !' : 'Enregistrer la note de séance'}
              </button>
              <button
                style={{ width: '100%', padding: '0.7rem', borderRadius: 'var(--radius-lg)', background: 'var(--color-success-bg)', border: '1.5px solid var(--color-success-border)', color: 'var(--color-success-deeper)', fontWeight: 700, fontSize: '0.88rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
                onClick={() => {
                  handleSaveNote()
                  setFicheBackStep('database')
                  setStep('fiche_exercice')
        ...
        return (
          <div style={{ minHeight: '100vh', background: 'var(--background)', paddingBottom: '6rem' }}>
            <div style={{ position: 'sticky', top: 0, zIndex: 100, background: 'var(--surface)', borderBottom: '1px solid var(--border-color)', padding: '0.75rem 1rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              ...
              <span style={{ fontSize: '0.7rem', fontWeight: 700, padding: '0.2rem 0.6rem', borderRadius: 'var(--radius-full)', background: 'var(--color-orange-bg)', color: 'var(--color-orange-dark)', border: '1px solid var(--color-orange-border)' }}>Intermédiaire</span>
            </div>
            ...
            <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: 'var(--surface)', borderTop: '1px solid var(--border-color)', padding: '0.75rem 1rem', display: 'flex', flexDirection: 'column', gap: 8, zIndex: 100 }}>
              <button
                style={{ width: '100%', padding: '0.85rem', borderRadius: 'var(--radius-lg)', background: 'linear-gradient(135deg, var(--color-orange), var(--color-orange-dark))', border: 'none', color: 'white', fontWeight: 700, fontSize: '0.95rem', cursor: 'pointer' }}
