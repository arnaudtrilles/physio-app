...
/**
 * Wrapper sécurisé autour de callGemini.
 *
 * Objectifs :
...
export async function callGeminiSecure(opts: SecureCallOptions): Promise<string> {
  const {
  ...
  // Si des documents sans flag `masked=true` sont joints, on refuse l'envoi
  // sauf si l'utilisateur a explicitement validé (userAcknowledgedUnmasked).
    ...
    if (onAudit) {
      try {
        onAudit({
          ...
          documentsCount,
          documentsUnmasked,
          modelUsed: preferredModel ?? 'gemini (fallback chain)',
          promptLength,
