...
/**
 * Vue détail d'un patient : en-tête, carte héro, suivi des prescriptions
 * (masqué pour le lancement v1), objectifs SMART, et la section d'évolution
 * par zone/épisode (graphiques EVN/EVA, timeline chronologique, actions PDF /
 * analyse / fiche / documents) plus le sélecteur de consultation.
 *
 * Extraction verbatim de la branche `selectedPatient` de DatabasePage (rendu et
 * comportement identiques). Toutes les données proviennent de
...
export function PatientDetailView() {
  ...
  // Cette vue n'est montée que lorsqu'un patient est sélectionné (cf.
  // DatabasePage). La garde rend ce contrat explicite et narrow `selectedPatient`
  // à `string` pour tout le corps — aucun changement de comportement.
                          ...
                          return (
                            <div key={record.id} style={{ background: 'var(--surface)', borderRadius: 'var(--radius-lg)', border: `1.5px solid ${bilanOpen ? 'var(--border-soft)' : 'var(--info-bg)'}`, boxShadow: bilanOpen ? 'var(--shadow-sm)' : 'none', overflow: 'hidden' }}>
                              ...
                              {(() => {
                                ...
                                // Index par id pour afficher "Intermédiaire N°X" (position chronologique, sortie exclue)
                                const intermIndexById = new Map<number, number>()
                                let interIdx = 0
                                zoneInters.forEach(r => {
                                  if (r.type === 'sortie') return
                                  intermIndexById.set(r.id, interIdx)
                                  interIdx++
                                })
                                ...
                                return (
                                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginTop: '0.5rem' }}>
                                    {timeline.map(item => {
                                      ...
                                      return (
                                      <div key={rec.id} style={{ background: 'var(--surface)', border: `1.5px solid ${rec.type === 'sortie' ? (intermOpen ? '#86efac' : '#bbf7d0') : (intermOpen ? '#fdba74' : '#fed7aa')}`, borderRadius: 'var(--radius-lg)', boxShadow: intermOpen ? 'var(--shadow-sm)' : 'none', overflow: 'hidden' }}>
                                        <div
                                          role="button"
                                        ...
                                        >
                                          <div style={{ flex: 1, minWidth: 0 }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.45rem', flexWrap: 'wrap' }}>
                                              <span style={{ fontWeight: 700, color: rec.type === 'sortie' ? '#166534' : '#92400e', fontSize: '0.85rem' }}>{rec.type === 'sortie' ? 'Bilan de sortie' : `Intermédiaire n°${idx + 1}`}</span>
                                              {rec.status === 'incomplet' && <span style={{ fontSize: '0.66rem', fontWeight: 700, color: '#991b1b' }}>Incomplet</span>}
                                            </div>
                                            <div style={{ fontSize: '0.72rem', color: rec.type === 'sortie' ? '#15803d' : '#c2410c', marginTop: 1 }}>{rec.dateBilan}</div>
                                          </div>
                                          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                                            {score !== null && (
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, padding: '0 0.9rem 0.75rem' }}>
                                          {rec.status === 'incomplet' ? (
                                            ...
                                            <>
                                              {/* Rangée 1 : Bilan PDF + Note diag (Note diag masquée pour sortie) */}
                                              <div style={{ display: 'flex', gap: 6 }}>
                                                <button
                                                  ...
                                                  onClick={() => {
                                                    // Exporte le bilan intermédiaire/sortie comme un BilanRecord pour réutiliser exportBilanFromRecord
                                                    exportBilanFromRecord({
                                                    ...
                                                    } as BilanRecord, rec.type === 'sortie' ? 'sortie' : 'intermediaire')
                                                  }}
                                              ...
                                              {/* Rangée 2 : Fiche d'exercices (masquée pour sortie) */}
                                              {rec.type !== 'sortie' && (
                                                <button
