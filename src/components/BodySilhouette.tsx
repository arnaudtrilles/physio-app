...
const DrawingCanvas = ({
  ...
  return (
    <div style={{ userSelect: 'none' }}>
      ...
      <div style={{ position: 'relative', borderRadius: 10, overflow: 'hidden', border: '1.5px solid var(--border-color)', background: '#f8fafc' }}>
        <svg
...
const BodyZoneSVG = ({ highlightZone }: { highlightZone: string }) => {
  const zp = (id: string) => ({
    fill: id === highlightZone ? '#bfdbfe' : '#dde6ee',
    stroke: id === highlightZone ? '#1e40af' : '#7a8fa6',
...
export const BodySilhouette = ({ onContextChange }: { onContextChange: (data: any) => void }) => {
  ...
  const getColor = (zone: string) => {
    ...
    if (!d || !d.saved) return '#dde6ee';
    const i = parseInt(d.intensite || '5', 10);
    if (i === 0) return '#86efac';
    if (i <= 2) return '#fde047';
    if (i <= 4) return '#fb923c';
    if (i <= 6) return '#f87171';
    if (i <= 8) return '#dc2626';
    return '#991b1b';
  };

  const zp = (id: string) => ({
    fill: getColor(id),
    stroke: selectedZone === id ? '#1e40af' : '#7a8fa6',
  ...
  const MicBtn = ({ field }: { field: string }) => (
    <button className={`mic-btn-inline ${activeField === field ? 'recording' : ''}`} onClick={() => handleVoice(field)}>
      {activeField !== field && (
