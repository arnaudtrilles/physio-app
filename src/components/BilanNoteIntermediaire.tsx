  ...
  return (
    <div className="general-info-screen slide-in-left">
      <header className="screen-header">
        ...
        <div style={{ background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: 8, padding: '4px 10px', fontSize: 10, fontWeight: 700, color: '#c2410c' }}>
          Intermédiaire
        </div>
      </header>
      ...
      <div className="scroll-area" style={{ paddingBottom: '6rem' }}>
        ...
        {/* Hero */}
        <div className="ai-hero" style={{ background: 'linear-gradient(135deg, #ea580c, #c2410c)' }}>
        ...
        {error && error !== 'quota' && error !== 'auth' && (
          <div style={{ background: '#fef2f2', border: '1px solid #fca5a5', borderRadius: 12, padding: 16, marginBottom: 12 }}>
            ...
            <button onClick={() => { setRetryCount(0); run(0) }} style={{ marginTop: 8, fontSize: '0.82rem', color: 'var(--primary)', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>Réessayer</button>
          </div>
        )}
          ...
          <div className="fade-in-up">
            ...
            <div className="ai-section-card">
              <div className="ai-section-header">
                ...
                <h4 style={{ color: '#9a3412' }}>Note diagnostique</h4>
              </div>
              <div className="ai-section-body">
                <div className="ai-diagnostic-box" style={{ borderLeft: '4px solid #ea580c' }}>
                  ...
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '0.3rem 0.75rem', background: '#fff7ed', borderRadius: 8, border: '1px solid #fed7aa', fontSize: '0.72rem', fontWeight: 600, color: '#c2410c' }}>
                    <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#c2410c" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
            ...
            <div className="ai-section-card">
              <div className="ai-section-header">
                ...
                <h4 style={{ color: '#166534' }}>Prise en charge ajustée</h4>
              </div>
              <div className="ai-section-body">
                {note.priseEnChargeAjustee.map((item, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '0.6rem 0', borderBottom: i < note.priseEnChargeAjustee.length - 1 ? '1px solid var(--border-color)' : 'none' }}>
                    <div style={{ width: 22, height: 22, borderRadius: '50%', background: 'linear-gradient(135deg, #16a34a, #15803d)', color: 'white', fontWeight: 700, fontSize: '0.72rem', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 1 }}>
                      {i + 1}
                    </div>
        ...
        {note && !loading && (
          <div style={{ marginTop: 4, marginBottom: 8 }}>
            <button
              ...
              style={{ width: '100%', padding: '0.6rem 1rem', borderRadius: 10, background: showCorrection ? '#fff7ed' : 'var(--secondary)', border: `1.5px solid ${showCorrection ? '#fed7aa' : 'var(--border-color)'}`, color: showCorrection ? '#92400e' : 'var(--text-muted)', fontWeight: 600, fontSize: '0.82rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
            ...
            {showCorrection && (
              <div className="fade-in" style={{ marginTop: 8, background: '#fefce8', border: '1.5px solid #fde68a', borderRadius: 12, padding: '0.85rem' }}>
                ...
                <button
                  ...
                  style={{ marginTop: 8, width: '100%', padding: '0.7rem', borderRadius: 10, background: !correction.trim() ? 'var(--secondary)' : 'linear-gradient(135deg, #ea580c, #c2410c)', border: 'none', color: !correction.trim() ? 'var(--text-muted)' : 'white', fontWeight: 700, fontSize: '0.85rem', cursor: !correction.trim() ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, opacity: refining ? 0.7 : 1 }}>
                  {refining ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {!note && !loading && apiKey && !error && (
            <button className="btn-primary-luxe" style={{ marginBottom: 0, background: 'linear-gradient(135deg, #ea580c, #c2410c)' }} onClick={() => run(0)}>
              Générer la note diagnostique
            </button>
          )}
          {loading && (
            <button className="btn-primary-luxe" disabled style={{ marginBottom: 0, opacity: 0.7, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, background: 'linear-gradient(135deg, #ea580c, #c2410c)' }}>
          ...
          {note && (
            <button
              ...
              style={{ width: '100%', padding: '0.85rem', borderRadius: 'var(--radius-lg)', background: '#f0fdf4', border: '1.5px solid #bbf7d0', color: '#15803d', fontSize: '0.9rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
