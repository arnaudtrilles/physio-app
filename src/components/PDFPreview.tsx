  ...
  return (
    <div>
      {lines.map((line, i) => {
        ...
        if (line.startsWith('### '))
          return <div key={i} style={{ fontWeight: 800, fontSize: '1rem', color: '#1e3a8a', marginTop: 16, marginBottom: 8, padding: '6px 10px', background: '#eff6ff', borderRadius: 6, borderLeft: '3px solid #1e3a8a' }}>{renderBold(line.slice(4))}</div>
        if (line.startsWith('## '))
          return <div key={i} style={{ fontWeight: 700, fontSize: '1.05rem', color: '#1e3a8a', marginTop: 14, marginBottom: 6 }}>{renderBold(line.slice(3))}</div>
        if (line === '---')
        ...
        if (line.startsWith('- '))
          return <div key={i} style={{ fontSize: '0.85rem', color: 'var(--text-main)', margin: '3px 0', lineHeight: 1.5, display: 'flex', gap: 6, paddingLeft: 4 }}><span style={{ color: '#1e3a8a', fontWeight: 700, flexShrink: 0 }}>•</span><span>{renderBold(line.slice(2))}</span></div>
