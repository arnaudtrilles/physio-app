    ...
    return canAdd ? (
      <div>
        <button
          ...
          style={{ width: '100%', padding: '0.7rem', borderRadius: 10, border: '2px dashed #4A8C73', background: '#edf4f1', color: '#2D5A4B', fontWeight: 600, fontSize: '0.85rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
    ...
    <div>
      ...
      {enCours.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 10 }}>
          {enCours.map(obj => (
            <div key={obj.id} style={{ background: '#edf4f1', border: '1px solid #b8d4ca', borderRadius: 10, padding: '0.7rem' }}>
              {editingId === obj.id ? (
                <div>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button onClick={() => {
                      ...
                      setEditingId(null)
                    }} style={{ flex: 1, padding: '0.35rem', borderRadius: 6, border: 'none', background: '#2D5A4B', color: 'white', fontWeight: 700, fontSize: '0.75rem', cursor: 'pointer' }}>
                      OK
                    </button>
                ...
                <>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 }}>
                    <div>
                      <div style={{ fontWeight: 700, fontSize: '0.85rem', color: '#2D5A4B' }}>{obj.titre}</div>
                      <div style={{ fontSize: '0.78rem', color: '#4A8C73' }}>{obj.cible}</div>
                    </div>
                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                      {obj.dateCible && <div style={{ fontSize: '0.7rem', color: '#4A8C73', fontWeight: 600 }}>{obj.dateCible}</div>}
                      <button onClick={() => { setEditingId(obj.id); setEditTitre(obj.titre); setEditCible(obj.cible); setEditDate(obj.dateCible) }}
                        style={{ fontSize: '0.62rem', color: '#2D5A4B', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer', padding: 0, marginTop: 1 }}>
                        Modifier
                      </button>
                    </div>
                  </div>
      ...
      {/* Add form */}
      {showAdd ? (
        <div style={{ background: '#edf4f1', border: '1.5px solid #b8d4ca', borderRadius: 10, padding: '0.75rem', marginBottom: 6 }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <button onClick={handleAdd} disabled={!titre.trim() || !cible.trim()}
              style={{ flex: 1, padding: '0.55rem', borderRadius: 8, background: titre.trim() && cible.trim() ? 'linear-gradient(135deg, #2D5A4B, #1A3830)' : 'var(--secondary)', border: 'none', color: titre.trim() && cible.trim() ? 'white' : 'var(--text-muted)', fontWeight: 700, fontSize: '0.85rem', cursor: titre.trim() && cible.trim() ? 'pointer' : 'not-allowed' }}>
              Ajouter
            </button>
        ...
        <button onClick={() => setShowAdd(true)}
          style={{ width: '100%', padding: '0.55rem', borderRadius: 8, border: '1.5px dashed #4A8C73', background: '#edf4f1', color: '#2D5A4B', fontWeight: 600, fontSize: '0.82rem', cursor: 'pointer' }}>
          + Ajouter un objectif
        </button>
      ) : null}
    </div>
